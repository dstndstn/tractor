...
int ac; char *av[];
{
  ...
  if(ac<2){
    printf("\n\t*****  z_9_ccd  X,Y --> R.A.,Dec. *****\n");
    printf("\n\t Usage: z_9xy2ad fitsfile x1 y1 [x2 y2 ...]");
    printf("\n\t        z_9xy2ad batch_file\n");
    printf("\n\t in batch file:");
    printf("\n\t   fitsfile_name");
    printf("\n\t   epoch");
...
l30:
    ...
    sscanf(a,"%lf %lf",&x,&y);
    xy_rad(a8[6],a8[7],x,y,&xa,&xd,1,0);
    astprs2(xa,xd,2000.,&alpha,&delta,epoch);
    toms2(alpha,c1,1);  toms2(delta,c2,0);
    printf("%9.2lf %9.2lf  ---> %s %s (%6.1lf)\n",x,y,c1,c2,epoch);
    goto l30;
  } 
  k=2;
l10:
  sscanf(av[k++],"%lf",&x); 
  sscanf(av[k++],"%lf",&y); 
  xy_rad(a8[6],a8[7],x,y,&alpha,&delta,1,0);
  toms2(alpha,c1,1);  toms2(delta,c2,0);
  printf("%9.2lf %9.2lf  ---> %s %s (%6.1lf)\n",x,y,c1,c2,epoch);
  if(ac>k+1)goto l10;
}
