...
def s82_main():
    ...
    T[:Nkeep].writeto(os.path.join(outdir, 'stamps.fits'),
                      columns='''tag objid run camcol field ra dec psfmag_u psfmag_g psfmag_r psfmag_i psfmag_z modelmag_u modelmag_g modelmag_r modelmag_i modelmag_z'''.split())
    

    #write_catalogs(T, Nkeep, outdir)
    ...
    for ra,dec in zip(T.ra, T.dec)[:Nkeep]:
        oneblob(ra, dec, [], cutToPrimary, outdir)
        
    
def write_catalogs(T, Nkeep, outdir):
...
def oneblob(ra, dec, addToHeader, cutToPrimary, outdir):
    ...
    for band in bands:
        ...
        for ifield,(run,camcol,field) in enumerate(RCF):
            ...
            fx,fy = tim.getWcs().positionToPixel(RaDecPos(ra, dec))
            x,y = int(fx), int(fy)
