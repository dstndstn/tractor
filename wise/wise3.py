...
def stage100(opt=None, ps=None, ralo=None, rahi=None, declo=None, dechi=None,
    ...
    roipoly = np.array([(ralo, declo), (ralo, dechi),
                        (rahi, dechi), (rahi, declo)])
    ...
    for d, tag in wisedatadirs:
        if indexfn is None:
            ...
            ifn = indexfn
        T = fits_table(ifn, columns=['ra', 'dec',
        ...
        assert(len(np.unique([s + '%03i' % f for s,
                              f in zip(T.scan_id, T.frame_num)])) == len(T))
    ...
    if ps:
        ...
        plot = Plotstuff(outformat='png', ra=(r0 + r1) / 2.,
                         dec=(d0 + d1) / 2., width=d1 - d0, size=(800, 800))
        out = plot.outline
        plot.color = 'white'
        plot.alpha = 0.07
        plot.apply_settings()
    ...
    if opt.wsources is not None and os.path.exists(opt.wsources):
        ...
        W.cut((W.ra > ralo) * (W.ra < rahi) *
              (W.dec > declo) * (W.dec < dechi))
    else:
...
def stage104(opt=None, ps=None, ralo=None, rahi=None, declo=None, dechi=None,
    ...
    ra = (ralo + rahi) / 2.
    dec = (declo + dechi) / 2.
    W, H = (rahi - ralo) * np.cos(np.deg2rad(dec)) / \
        pixscale, (dechi - declo) / pixscale
...
def stage106(opt=None, ps=None, tractor=None, band=None, bandnum=None, T=None,
    ...
    try:
        ...
        for tim in tims:
            plt.clf()
            plt.imshow(tim.getImage(), interpolation='nearest',
                       origin='lower', cmap='gray')
            plt.title('Image ' + tim.name)
            ps.savefig()

            plt.clf()
            plt.imshow(tim.getInvvar(), interpolation='nearest',
                       origin='lower', cmap='gray')
            plt.colorbar()
    ...
    R.inblock = np.hstack(
        (S.inblock, np.array([1] * len(UW)))).astype(np.uint8)
...
def _resample_one(argl):
    ...
    if mod is not None:
        inims.append(mod)
    try:
        yo, xo, yi, xi, rpix = resample_with_wcs(
            targetwcs, wcs2, inims, Lorder, spline=spline)
    except OverlapError:
        return None
    rpatch = np.zeros((H, W))
    rpatch[yo, xo] = rpix[0]
...
def _resample_mod(args):
    ...
    try:
        yo, xo, yi, xi, rpix = resample_with_wcs(
            targetwcs, wcs2, [mod], Lorder, spline=spline)
    except OverlapError:
        return None
    rmod = np.zeros((H, W))
    rmod[yo, xo] = rpix[0]
...
def stage108(opt=None, ps=None, ralo=None, rahi=None, declo=None, dechi=None,
    ...
    try:
        ...
        plt.clf()
        plt.imshow(coinvvar, interpolation='nearest',
                   origin='lower', cmap='gray')
        plt.colorbar()
        plt.title('Coinvvar')
        ps.savefig()
    ...
    for s2, s3 in zip(cat2, cat3):
        m2.append(NanoMaggies.nanomaggiesToMag(
            s2.getBrightness().getBand(band)))
        m3.append(NanoMaggies.nanomaggiesToMag(
            s3.getBrightness().getBand(band)))
    m2 = np.array(m2)
    m3 = np.array(m3)
...
def stage109(opt=None, ps=None, ralo=None, rahi=None, declo=None, dechi=None,
    ...
    if ps:
        ...
        for cat4 in cats4:
            m4 = []
            for s in cat4:
                m4.append(NanoMaggies.nanomaggiesToMag(
                    s.getBrightness().getBand(band)))
...
def stage205(opt=None, ps=None, tractor=None, band=None, bandnum=None, T=None,
    ...
    if ps is not None:
        ...
        imchis = [dict(interpolation='nearest', origin='lower',
                       vmin=-5, vmax=5)] * len(ptims)
...
def stage204(opt=None, ps=None, tractor=None, band=None, bandnum=None, **kwa):
    ...
    if minFlux is not None:
        minFlux = np.median(
            [tim.sigma1 * minFlux / tim.getPhotoCal().val for tim in tims])
        print('minFlux:', minFlux, 'nmgy')
...
def stage306(opt=None, ps=None, tractor=None, band=None, bandnum=None, rd=None, **kwa):
    ...
    for ti, tim in enumerate(tims):
        ...
        imchis = [dict(interpolation='nearest',
    ...
    for coi, pixscale in enumerate([2.75 / 3600., 0.4 / 3600]):
        ...
        for sband in ['u', 'g', 'r', 'i', 'z']:
            tim, inf = get_tractor_image_dr9(
                r, c, f, sband, psf='dg', nanomaggies=True)
            mn = inf['sky']
            st = inf['skysig']
...
def stage404(coadds=None, ps=None, targetrd=None,
    ...
    r, c, f = rcf
    stim, inf = get_tractor_image_dr9(
        r, c, f, sband, psf='dg', nanomaggies=True)
    sig1 = inf['skysig']
...
def stage510(S=None, W=None, targetrd=None, **kwa):
    for cat in [1, 2, 3, 4, 5]:
        ...
        I, J, d = match_radec(Si.ra, S.dec, np.array(
            [r]), np.array([d]), 1. / 3600.)
        print('Matched', len(I))
...
def stage606():
    ...
    for i, (mask, tim) in enumerate(zip(rmasks, tims)):
        ...
        plt.subplot(R, C, 5)
        plt.imshow(np.abs(rchi) > 5, interpolation='nearest',
                   origin='lower', cmap='gray')
        plt.title('abs(rchi) > 5')

        plt.subplot(R, C, 6)
        plt.imshow(d.mask, interpolation='nearest',
                   origin='lower', cmap='gray')
        plt.title('mask')
...
def stage701(opt=None, ps=None, ralo=None, rahi=None, declo=None, dechi=None,
    ...
    if ps:
        ...
        for s1, s2 in zip(tractor.getCatalog(), cat):
            m1.append(NanoMaggies.nanomaggiesToMag(
                s1.getBrightness().getBand(band)))
            m2.append(NanoMaggies.nanomaggiesToMag(
                s2.getBrightness().getBand(band)))
        m1 = np.array(m1)
        m2 = np.array(m2)
    ...
    R.set('coadd_' + band,
          np.array([src.getBrightness().getBand(band) for src in cat]))
...
def main():
    ...
    if ri == -1 and True:
        ...
        T = fits_table('/home/schlegel/wise1ext/sdss/zans-plates7027-7032-zscan.fits',
        ...
        qsoscan = 'QSO    '
        I = np.flatnonzero((T.zwarning == 0) * (T.clazz ==
                                                qso) * (T.typescan == qsoscan))
        print(len(I), 'QSO, scan QSO, with Zwarning = 0')

        for zcut in [2, 2.3, 2.5]:
            I = np.flatnonzero((T.zwarning == 0) * (T.clazz == qso)
                               * (T.typescan == qsoscan) * (T.z > zcut))
            print(len(I), 'QSO, scan QSO, with Zwarning = 0, z >', zcut)

            I = np.flatnonzero((T.zwarning == 0) * (T.clazz == qso)
                               * (T.typescan == qsoscan) * (T.zscan > zcut))
        ...
        for j, i in enumerate(range(len(T))):
            ...
            for k, (rc, dc) in enumerate(zip(atlas.ra, atlas.dec)):
                ...
                if not os.path.exists(pngfn):
                    jfn = 'sdss-%i.jpg' % (j)
                    url = 'http://skyservice.pha.jhu.edu/DR10/ImgCutout/getjpeg.aspx?ra=%f&dec=%f&scale=%f&width=%i&height=%i' % (
                        rs, ds, pscale, SW, SH)
                    cmd = 'wget "%s" -O %s' % (url, jfn)
                    os.system(cmd)
                    cmd = 'jpegtopnm %s | pnmtopng > %s' % (
                        'sdss-%i.jpg' % j, 'sdss-%i.png' % j)
                    os.system(cmd)

                for band in [1, 2]:
                    fn = 'unwise-coadds/%s/%s/unwise-%s-w%i-img-m.fits' % (
                        co[:3], co, co, band)
                ...
                if os.path.exists(wfn):
                    W = fits_table(wfn)
                    W.cut(degrees_between(ra, dec, W.ra, W.dec)
                          < S * 2 * 2.75 / 3600.)
                ...
                if os.path.exists(pfn):
                    P = fits_table(pfn)
                    P.cut(degrees_between(ra, dec, P.ra, P.dec)
                          < S * 2 * 2.75 / 3600.)
                    ...
                    for band in [1, 2]:
                        ...
                        tim = Image(data=img, invvar=np.ones_like(img), psf=psf, wcs=twcs,
                                    sky=tsky, photocal=LinearPhotoCal(
                                        1., band=wanyband),
                                    domask=False)
                        cat = []
                        iskip = []
                ...
                plt.imshow(np.clip(sdss * 6, 0, 1),
                           interpolation='nearest', extent=imextent)
                # ax = plt.axis()
                # ok,x,y = cowcs.radec2pixelxy(ra, dec)
    ...
    class MyCaller(CallGlobal):
        def getkwargs(self, stage, **kwargs):
            ...
            if opt.ps is not None:
                kwa.update(ps=PlotSequence(opt.ps + '-s%i' %
                                           stage, format='%03i'))
            return kwa
