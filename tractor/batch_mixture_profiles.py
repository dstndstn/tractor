...
# Constant for -2 * pi**2 and -2 * pi
PI2_NEG2 = -19.739208802178716
PI_NEG2 = -6.283185307179586
...
@cp.fuse()
def fused_f_component(a, d, b, mu0, mu1, amp, v, w):
    # Calculate the real exponent: -2 * pi^2 * (av^2 + dw^2 + 2bvw)
    re_part = PI2_NEG2 * (a * (v**2) + d * (w**2) + 2.0 * b * v * w)
    
    # Calculate the imaginary exponent: -2 * pi * (mu0*v + mu1*w)
    im_part = PI_NEG2 * (mu0 * v + mu1 * w)
    
    # Result: amp * exp(re + i*im)
    # If mu0/mu1 are 0, im_part is 0, and exp(re + 0j) is just the real part.
    return amp * cp.exp(re_part + 1j * im_part)
...
###  Batch GPU version below ###
    ...
    def getFourierTransformImagesBatchGPU(self,v,w, zero_mean=False):
        ...
        F_5d = fused_f_component(
            a[:,:,:,cp.newaxis,cp.newaxis],
            d[:,:,:,cp.newaxis,cp.newaxis],
            b[:,:,:,cp.newaxis,cp.newaxis],
            mu[:,:,:,0,cp.newaxis,cp.newaxis],
            mu[:,:,:,1,cp.newaxis,cp.newaxis],
            self.amp[:,:,:,cp.newaxis,cp.newaxis],
            v[cp.newaxis, cp.newaxis, cp.newaxis, cp.newaxis, :],
            w[cp.newaxis, cp.newaxis, cp.newaxis, :, cp.newaxis]
        )
        del a, b, d
        Fsum = F_5d.sum(axis=2)
        del F_5d
        return Fsum
        """
        F = cp.exp(-2. * cp.pi**2 *
    ...
    def calculate_Fsum_nested_chunked(self, v, w, chunk_size_k=1, chunk_size_ni=None):
        ...
        for i_ni in range(num_ni_chunks):
            ...
            for i_k in range(num_k_chunks):
                ...
                # Using the fused kernel to calculate the 5D chunk in one pass
                f_5d_chunk = fused_f_component(
                    ...
                    mu[s_ni:e_ni, :, s_k:e_k, 0, cp.newaxis, cp.newaxis],
                    mu[s_ni:e_ni, :, s_k:e_k, 1, cp.newaxis, cp.newaxis],
                    amp[s_ni:e_ni, :, s_k:e_k, cp.newaxis, cp.newaxis],
                    v[cp.newaxis, cp.newaxis, cp.newaxis, cp.newaxis, :],
                    w[cp.newaxis, cp.newaxis, cp.newaxis, :, cp.newaxis]
                )

                Fsum_batch += f_5d_chunk.sum(axis=2)
                del f_5d_chunk
                
            # Move batch into final total
            Fsum_total[s_ni:e_ni] = Fsum_batch
            del Fsum_batch
            
            # Force cleanup before next image batch
