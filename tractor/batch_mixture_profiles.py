    ...
    def __init__(self, amixes, IM, IF, K, D, mogweights, fftweights, px, py, mux, muy, mmpix, mmie, mh, mw, counts, cdi, roi):
        ...
        if self.maxNmogs > 0:
            ...
            for i, amix in enumerate(amixes):
                ...
                amp[:,i][flipped_mask] = (cp.tile(mix.amp, (self.Nimages,1)) * mogweights[:,None])[IM]
                mean[:,i][flipped_mask] = (cp.tile(mix.mean, (self.Nimages, 1, 1)) + cp.array([px,py]).T[:,cp.newaxis,:])[IM]
                var[:,i][flipped_mask] = mix.var[IM]
...
class ImageDerivs(object):
    ...
    def __init__(self, amixes, IM, IF, K, D, mogweights, fftweights, px, py, mux, muy, mmpix, mmie, mh, mw, counts, cdi, roi):
        ...
        self.mux = mux
        self.muy = muy
        self.mmpix = np.asarray(mmpix)
        self.mmie = np.asarray(mmie)
        self.mh = mh
        self.mw = mw
        self.counts = counts
        self.cdi = cdi
...
class BatchImageParams(object):
    ...
    def collect_params(self):
        print ("COLLECT", len(self.img_derivs))
        t = time.time()
        assert(self._initialized is False)
        self._initialized = True
        ...
        self.counts = cp.asarray([imderiv.counts for imderiv in self.img_derivs])
        self.cdi = cp.asarray([imderiv.cdi for imderiv in self.img_derivs])
        self.roi = np.asarray([imderiv.roi for imderiv in self.img_derivs], dtype=np.int)
        self.steps = cp.asarray([imderiv.steps for imderiv in self.img_derivs])
        tb[0] += time.time()-t
        t = time.time()

        #self.pix = cp.asarray([np.pad(imderiv.mmpix, ((0, self.mh - imderiv.mmpix.shape[0]), (0, self.mw - imderiv.mmpix.shape[1])), mode='constant',constant_values=(np.float32(0.0),)) for imderiv in self.img_derivs])
        #self.ie = cp.asarray([np.pad(imderiv.mmie, ((0, self.mh - imderiv.mmie.shape[0]), (0, self.mw - imderiv.mmie.shape[1])), mode='constant',constant_values=(np.float32(0.0),)) for imderiv in self.img_derivs])
        #self.pix = np.zeros((self.Nimages, self.mh, self.mw), dtype = np.float32 )
        #self.ie = np.zeros_like(self.pix)
        self.pix = cp.asarray([imderiv.mmpix for imderiv in self.img_derivs])
        self.ie = cp.asarray([imderiv.mmie for imderiv in self.img_derivs])
        ...
        #for i, imderiv in enumerate(self.img_derivs):
        #    t = time.time()
        #    print ("ID", imderiv.mmpix.shape, self.mh, self.mw)
        #    self.pix[i,:self.mh,:self.mw] = imderiv.mmpix
        #    self.ie[i,:self.mh,:self.mw] = imderiv.mmie
        ...
        #tb[2] += time.time()-t
        #t = time.time()
        #self.pix = cp.asarray(self.pix)
        #self.ie = cp.asarray(self.ie)
        #tb[7] += time.time()-t
        t = time.time()
