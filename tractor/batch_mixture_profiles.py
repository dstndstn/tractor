...
# Pre-calculate the constant once
NEG_2_PI_SQ = -2.0 * (cp.pi**2)
# Using the exact same constant to ensure bit-perfect matching
NEG_2_PI_1J = -2.0 * cp.pi * 1j
...
@cp.fuse()
def fused_real_F(a, d, b, v, w):
    """
    Computes ONLY:
    exp(-2 * pi^2 * (a*v^2 + d*w^2 + 2*b*v*w))
    """
    # math: exp(constant * (term1 + term2 + term3))
    exponent = NEG_2_PI_SQ * (a * (v**2) + d * (w**2) + 2.0 * b * v * w)
    # Cast to complex64 immediately so the output is ready for Fsum
    return cp.exp(exponent).astype(cp.float32)

@cp.fuse()
def fused_apply_phase_and_amp(F_real, mu0, mu1, amp, v, w):
    """
    Computes: amp * F_real * exp(-2 * pi * i * (mu0*v + mu1*w))
    Combines phase shift and amplitude scaling into one pass.
    """
    phase = cp.exp(NEG_2_PI_1J * (mu0 * v + mu1 * w))
    return amp * F_real * phase

@cp.fuse()
def fused_apply_amp_only(F_real, amp):
    """
    Computes: amp * F_real
    Used when there is no phase shift (mu=0).
    """
    return amp * F_real
...
###  Batch GPU version below ###
    ...
    def getFourierTransformImagesBatchGPU(self,v,w, zero_mean=False):
        ...
        if sz_gb > 5:
            ...
            return self.calculate_Fsum_nested_chunked(v, w, chunk_size_k=1, chunk_size_ni=2)
        
        elif sz_gb > 1:
            # Medium cases: Component chunking only
            # We reuse the nested logic but with Ni chunk = total Ni
            return self.calculate_Fsum_nested_chunked(v, w, chunk_size_k=1, chunk_size_ni=a.shape[0])
            ##Chunk large arrays
        ...
        #n, K
        #5D arrays are fun!
        # Small arrays: Direct Fused Calculation
        # Still use the fuser here to avoid intermediate real/imag/amp temporary arrays
        # --- Standard Small Array Path ---
        v_grid = v[cp.newaxis, cp.newaxis, cp.newaxis, cp.newaxis, :]
        w_grid = w[cp.newaxis, cp.newaxis, cp.newaxis, :, cp.newaxis]

        # Calculate Real part
        F = fused_real_F(
            a[:,:,:,cp.newaxis,cp.newaxis],
            d[:,:,:,cp.newaxis,cp.newaxis],
            b[:,:,:,cp.newaxis,cp.newaxis],
            v_grid, w_grid
        )
        del a, b, d
        
        # Apply Complex part
        mu0, mu1 = mu[:,:,:,0,cp.newaxis,cp.newaxis], mu[:,:,:,1,cp.newaxis,cp.newaxis]
        amp = self.amp[:,:,:,cp.newaxis,cp.newaxis]
        if (mu0 != 0).any() or (mu1 != 0).any():
            # Promote to complex64 for the phase shift
            F = fused_apply_phase_and_amp(F.astype(cp.complex64), mu0, mu1, amp, v_grid, w_grid)
        else:
            # Stay real until the very end, then scale by amp
            F = fused_apply_amp_only(F, amp).astype(cp.complex64)
    ...
    def calculate_Fsum_nested_chunked(self, v, w, chunk_size_k=1, chunk_size_ni=None):
        mu = self.mean
        a = self.var[:,:,:,0,0]
        b = self.var[:,:,:,0,1]
        d = self.var[:,:,:,1,1]
        ...
        # Grid setup for fuser broadcasting
        v_grid = v[cp.newaxis, cp.newaxis, cp.newaxis, cp.newaxis, :]
        w_grid = w[cp.newaxis, cp.newaxis, cp.newaxis, :, cp.newaxis]

        Fsum_total = cp.zeros((Ni, Nd, nw, nv), dtype=cp.complex64)

        # --- Dynamic Chunking Logic ---
        if chunk_size_ni is None:
            # Get free memory in bytes
            free_mem, _ = cp.cuda.Device().mem_info
            # Safety factor: leave 20% of memory for the OS and small buffers
            available_gb = (free_mem * 0.8) / 1e9

            # Estimate size of one image chunk (1 image, all Nd, 1 K, all v, all w)
            # 1 * Nd * 1 * nw * nv * 8 bytes (complex64)
            one_ni_k_chunk_gb = (Nd * nw * nv * 8) / 1e9

            # Calculate how many images we can fit in the available space
        ...
        for i_ni in range(num_ni_chunks):
            s_ni, e_ni = i_ni * chunk_size_ni, min((i_ni + 1) * chunk_size_ni, Ni)
            Fsum_batch = cp.zeros((e_ni - s_ni, Nd, nw, nv), dtype=cp.complex64)

            for i_k in range(num_k_chunks):
                ...
                # 2. Apply Complex Phase Shift (only if necessary)
                mu_c0 = mu[s_ni:e_ni, :, s_k:e_k, 0, cp.newaxis, cp.newaxis]
                mu_c1 = mu[s_ni:e_ni, :, s_k:e_k, 1, cp.newaxis, cp.newaxis]
                amp_c = amp[s_ni:e_ni, :, s_k:e_k, cp.newaxis, cp.newaxis]

                # Equivalent to z.any() but for the current chunk
                if (mu_c0 != 0).any() or (mu_c1 != 0).any():
                    F_c = fused_apply_phase_and_amp(F_c.astype(cp.complex64), mu_c0, mu_c1, amp_c, v_grid, w_grid)
                else:
                    # No shift: scale real part, then cast to complex for the sum
                    F_c = fused_apply_amp_only(F_c, amp_c).astype(cp.complex64)

                Fsum_batch += F_c.sum(axis=2)

                del F_c, mu_c0, mu_c1, amp_c
                cp.get_default_memory_pool().free_all_blocks()

            Fsum_total[s_ni:e_ni] = Fsum_batch
            del Fsum_batch
            cp.get_default_memory_pool().free_all_blocks()
